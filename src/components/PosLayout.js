    ...
    <div className="pos-shell">
      <header className="pos-topbar">
        <div className="pos-topbar-left">
          <div className="pos-logo" onClick={() => navigate('/')} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigate('/')}>
            <span className="pos-logo-icon"><i className="bi bi-bag-check-fill"></i></span>
            <span className="pos-logo-text">Bright Mart <small>POS</small></span>
          </div>
        ...
        <div className="pos-topbar-right">
          ...
          <div className="pos-user-pill">
            <span className="pos-user-avatar">{user?.name?.[0]?.toUpperCase() || 'U'}</span>
            <span className="d-none d-md-inline">{user?.name || 'Cashier'}</span>
          </div>
        </div>
      </header>
