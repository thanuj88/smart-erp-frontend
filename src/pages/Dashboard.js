...
const Dashboard = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      const [daily, recentData] = await Promise.all([
      ...
      <div className="row g-3 mb-3">
        <div className="col-12 col-sm-6 col-xl-3">
          <div className="metric-card metric-orange">
            <div className="metric-info">
              <small>Today's Cash Revenue</small>
              <h3>{formatMoney(todayRevenue)}</h3>
              <div className="metric-sub">Cash sales only</div>
            </div>
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <div className="metric-card metric-blue">
            <div className="metric-info">
              <small>Installment In</small>
              <h3>{formatMoney(todayCollections)}</h3>
              <div className="metric-sub">Down payments + collections</div>
            </div>
            <div className="metric-icon">
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <div className="metric-card metric-dark">
            <div className="metric-info">
              <small>Today's Actual Income</small>
              <h3>{formatMoney(todayActualIncome)}</h3>
              <div className="metric-sub">Cash + installment money</div>
            </div>
            <div className="metric-icon">
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <div className="metric-card metric-teal">
            <div className="metric-info">
              <small>Cash Sales Profit</small>
              <h3>{formatMoney(todayProfit)}</h3>
              <div className="metric-sub">On cash sales only</div>
            </div>
            <div className="metric-icon">
          ...
          <div className="card shadow-sm rounded-4">
            <div className="card-body">
              <div className="d-flex align-items-baseline gap-2 mb-2">
                ...
                <span className="text-muted small">{recentSales.length}</span>
              </div>
                    ...
                    {recentSales.length === 0 ? (
                      ...
                      recentSales.map((sale) => (
                        <tr key={sale.id}>
                          ...
                          <td>{sale.quantity}</td>
                          <td>
                            {sale.is_return || sale.return_flag ? (
                              <span className="badge rounded-pill bg-danger">Return</span>
                            ) : (
                          ...
                          <td className={`fw-semibold${sale.is_return || sale.return_flag ? ' text-danger' : ''}`}>
